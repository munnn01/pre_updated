#!/usr/bin/env python
"""Push V12: resume three full epochs, then evaluate all 1,010 val clips."""
...
TEMPLATE = REPO / "kaggle" / "longtrain_fullval_v12_cell.sh"
...
def render_cell(
    ...
    for name, value in {
        "expected_best_sha": expected_best_sha,
        "expected_last_sha": expected_last_sha,
    }.items():
    ...
    for source, target in {
        ...
        "__EXPECTED_BEST_SHA__": expected_best_sha,
        "__EXPECTED_LAST_SHA__": expected_last_sha,
    }.items():
...
def notebook(cell: str, codec: str) -> dict:
    return {
        "cells": [{
            "id": f"longtrain-fullval-v12-{codec}",
            "cell_type": "code",
...
def metadata(account: str, slug: str, codec: str, source_dataset: str | None) -> dict:
    if codec == "h264" and not source_dataset:
        raise ValueError("h264 continuation requires its account-local private dataset")
    if codec == "h264" and source_dataset.split("/", 1)[0] != account:
        raise ValueError("h264 private source dataset must be owned by the target account")
    if codec == "h265" and source_dataset:
        raise ValueError("h265 continuation must use the frozen public checkpoint")
    return {
        "id": f"{account}/{slug}",
        "title": slug,
        ...
        "dataset_sources": [KINETICS] + (
            [H265_DATASET] if codec == "h265" else [source_dataset]
        ),
        "kernel_sources": [],
        "competition_sources": [],
        "model_sources": [],
...
def main() -> None:
    ...
    print(f"[longtrain-fullval-v12] generated {push_dir} at {commit}")
    if args.write_only:
        return
    ...
    print(
        f"[longtrain-fullval-v12] pushing {handle} codec={args.codec} "
