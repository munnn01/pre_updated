...
from src.data.coco_det import letterbox_image, letterbox_params  # noqa: E402
from src.metrics.bd_rate import bd_rate  # noqa: E402
from src.metrics.detection import (  # noqa: E402
    coco_box as _coco_box,
    coco_map,
...
def load_coco(images_dir: Path, ann_file: Path, n: int, size: int, seed: int = 0):
    ...
    for im in imgs[:n]:
        ...
        w0, h0 = img.size
        img, _ = letterbox_image(img, size)
...
def scaled_gt(anns, size: int, orig_hw):
    ...
    h0, w0 = orig_hw
    scale, left, top, _, _ = letterbox_params(w0, h0, size)
    out = []
    for a in anns:
        x, y, w, h = a["bbox"]
        out.append({
            ...
            "bbox": [x * scale + left, y * scale + top, w * scale, h * scale],
        })
    return out
...
class Detector:
    ...
    def __init__(self, device, score_thresh: float = 0.05,
        ...
        ctor, weights = table[backbone]
        self.device = device
        self.score_thresh = score_thresh
        self.backbone = backbone
        self.model = ctor(weights=weights).to(device).eval()
...
def run(args) -> dict:
    ...
    det = Detector(device, backbone=args.detector_backbone)
    ...
    for codec_name in ("h264", "h265"):
        ...
        if args.bootstrap:
            entry["ci"] = paired_bootstrap_detection_bd(
                per_image,
                codec=codec_name,
                qps=qps,
                gt_by_id=gt_by_id,
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        choices=["fasterrcnn_resnet50_fpn", "fasterrcnn_mobilenet_v3_large_fpn"],
        help="frozen detector used to score every probe arm",
    )
