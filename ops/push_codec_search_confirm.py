#!/usr/bin/env python
"""Push one commit-pinned frozen-confirmation shard to a Kaggle account."""
...
from ops.push_rcts_pilot import account_environment, kaggle_command, metadata, target_is_active

TEMPLATE = REPO / "kaggle" / "codec_search_confirm_cell.sh"


def render_cell(commit: str, codec: str, shard: int) -> str:
    if not re.fullmatch(r"[0-9a-f]{40}", commit):
        raise ValueError("commit must be a lowercase 40-character SHA")
    if codec not in ("h264", "h265") or not 0 <= shard < 4:
...
def main() -> None:
    ...
    notebook = {
        "cells": [{"id": f"codec-search-confirm-{args.codec}-{args.shard}", "cell_type": "code",
    ...
    (push_dir / "kernel-metadata.json").write_text(json.dumps(metadata(args.account, args.slug)), encoding="utf-8")
    print(f"[confirm] generated {args.account}/{args.slug} codec={args.codec} shard={args.shard} at {args.commit}", flush=True)
    if args.write_only:
        return
    ...
    result = subprocess.run(kaggle_command() + ["kernels", "push", "-p", str(push_dir)],
                            capture_output=True, text=True, errors="replace", env=environment, check=False)
