#!/usr/bin/env python
"""Create and push a commit-pinned codec-search AR Kaggle notebook."""
...
TEMPLATE = REPO / "kaggle" / "codec_search_ar_cell.sh"
...
def main():
    ...
    cell = TEMPLATE.read_text(encoding="utf-8").replace("__REF__", args.commit).replace("__CODEC__", args.codec)
    notebook = {
        "cells": [{"id": f"codec-search-ar-{args.codec}", "cell_type": "code",
    ...
    print(f"[codec-search] generated {args.account}/{args.slug} at {args.commit}", flush=True)
    if args.write_only:
        return
