#!/usr/bin/env python
"""Generate and push one preregistered CAST-AR F1 Kaggle notebook."""
...
TEMPLATE = REPO / "kaggle" / "cast_ar_f1_cell.sh"
...
def notebook(cell: str, codec: str) -> dict:
    return {
        "cells": [{
            "id": f"cast-ar-f1-{codec}", "cell_type": "code",
...
def main() -> None:
    ...
    cell = render_cell(commit, args.codec, args.seed)
    ...
    print(f"[cast-ar-f1] generated {push_dir} at {commit}")
    if args.write_only:
        return
    ...
    print(
        f"[cast-ar-f1] pushing {handle} codec={args.codec} seed={args.seed} "
