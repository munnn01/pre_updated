...
def _loss_weights(cfg: dict) -> LossWeights:
    ...
    Keeping this separate prevents architecture-specific terms such as UP-VCM's
    ``rho`` saliency distillation from being silently accepted by YAML but left
...
def evaluate(cfg: dict, ckpt_path: str, out_dir: str | None = None) -> dict:
    ...
    if cfg["task"]["name"] == "object_detection":
        return _evaluate_detection(cfg, pre, codec, analyzer, out_dir)
...
def _evaluate_classification(cfg, pre, codec, analyzer, out_dir) -> dict:
    ...
    for batch in tqdm(loader, desc="eval"):
        ...
        # D1: gate the edit at eval too. Ground-truth action labels are not
        # available at deployment, so the saliency target is the analyzer's own
        # source-clip prediction. The old ``labels`` target leaked test labels
        # into preprocessing and made a gated result non-deployable.
        gate_mask = None
        if bool(cfg.get("model", {}).get("gate", True)) and bool(
                cfg.get("loss", {}).get("use_task_mask", True)):
            with torch.no_grad():
                pseudo_labels = analyzer.predict(clips).argmax(dim=1)
            gate_mask = task_saliency(analyzer, clips, pseudo_labels)
...
def _evaluate_detection(cfg, pre, codec, analyzer, out_dir) -> dict:
    """Evaluate COCO mAP curves for the same-codec OD preprocessing claim.

    This is intentionally separate from the classification evaluator: detection
    targets are lists of box dictionaries and the quality metric is dataset-level
    COCO mAP, not per-sample top-1 accuracy.
    """
    ...
    data_cfg = cfg["data"]
    ds = CocoDetDataset(
        index_json=data_cfg["index"],
        split=_evaluate_split(cfg),
        frame_size=data_cfg.get("frame_size", cfg["task"].get("image_size", 320)),
        train=False,
        max_items=ev.get("max_items"),
    )
    loader = DataLoader(
        ds,
        batch_size=ev.get("batch_size", 4),
        shuffle=False,
        num_workers=ev.get("num_workers", 2),
        collate_fn=collate_coco_det,
    )
    qps = [int(q) for q in ev.get("qp_list", [30, 35, 40, 45, 50])]
    have_ffmpeg = ffmpeg_available()
    if not have_ffmpeg:
        print("[eval-od] WARNING: ffmpeg not found -> no real-codec curves")

    methods = ["anchor", "prep"] + (["sandwich"] if hasattr(pre, "post_restore") else [])
    ...
    next_image_id = 1
    next_ann_id = 1
    score_thresh = float(cfg["task"].get("score_thresh", 0.05))

    def predictions(outputs, ids):
        batch_preds = []
        for image_id, output in zip(ids, outputs):
            ...
            preds = [
                ...
                for box, score, label in zip(
                    output["boxes"][keep], output["scores"][keep], output["labels"][keep]
                )
            ]
            batch_preds.append(preds)
        return batch_preds

    for clips, targets in tqdm(loader, desc="eval-od"):
        clips = clips.to(device)
        targets = [{k: v.to(device) for k, v in target.items()} for target in targets]
        batch_ids = list(range(next_image_id, next_image_id + len(targets)))
        next_image_id += len(targets)
        image_ids.extend(batch_ids)
        for image_id, target in zip(batch_ids, targets):
            anns = []
            for box, label in zip(target["boxes"], target["labels"]):
                xywh = coco_box(box)
                lab = int(label)
                category_ids.add(lab)
                anns.append({
                    "id": next_ann_id,
                    "image_id": image_id,
                    "category_id": lab,
        ...
        for codec_name in ("h264", "h265"):
            for qp in qps:
                ...
                standard = StandardCodec(
                    codec=codec_name, qp=qp, preset=ev.get("preset", "medium")
                )
                x_anchor, bpp_anchor = standard.compress_decompress_items(clips)
                x_prep, bpp_prep = standard.compress_decompress_items(x_pre)
                outputs = {
                    "anchor": analyzer.predict(x_anchor.to(device)),
                    "prep": analyzer.predict(x_prep.to(device)),
                }
                bpps = {"anchor": bpp_anchor, "prep": bpp_prep}
                if "sandwich" in methods:
                    if "codec" in pre.post_restore.__code__.co_varnames:
                        restored = pre.post_restore(x_prep, cond, codec=codec_name)
                    else:
                        restored = pre.post_restore(x_prep, cond)
                    outputs["sandwich"] = analyzer.predict(restored.to(device))
                    bpps["sandwich"] = bpp_prep
                for method in methods:
                    pred_batch = predictions(outputs[method], batch_ids)
                    slot = records[method].setdefault((codec_name, qp), {})
                    for image_id, bpp, preds in zip(batch_ids, bpps[method], pred_batch):
                        slot[image_id] = (float(bpp), preds)

    ann_meta = {
        "categories": [{"id": category_id, "name": str(category_id)}
                       for category_id in sorted(category_ids)]
    }
    curves = {}
    bootstrap_ci = {}
    if have_ffmpeg and image_ids:
        for codec_name in ("h264", "h265"):
            for method in methods:
                rates, maps = [], []
                for qp in qps:
                    slot = records[method][(codec_name, qp)]
                    rates.append(sum(value[0] for value in slot.values()) / len(slot))
                    preds = [pred for value in slot.values() for pred in value[1]]
                    maps.append(coco_map(preds, gt_by_id, image_ids, ann_meta)[0])
                name = codec_name if method == "anchor" else f"{method}+{codec_name}"
                curves[name] = {"keys": qps, "bpp": rates, "accuracy": maps}
            n_boot = int(ev.get("bootstrap", 0))
            if n_boot:
                bootstrap_ci[codec_name] = paired_bootstrap_detection_bd(
                    records,
                    codec=codec_name,
                    qps=qps,
                    gt_by_id=gt_by_id,
                    image_ids=image_ids,
                    ann_meta=ann_meta,
                    arms=[m for m in methods if m != "anchor"],
                    n_boot=n_boot,
                    seed=int(cfg.get("seed", 0)),
                )

    extra = {"bootstrap_ci": bootstrap_ci} if bootstrap_ci else None
    return _finalize(
        curves,
        out_dir,
